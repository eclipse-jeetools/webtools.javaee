...
import org.eclipse.jst.j2ee.webservices.WebServiceClientGenerator;
import org.eclipse.jst.j2ee.webservices.WebServicesClientDataRegistry;
...
public class ServiceReferenceCreationOperation extends ModelModifierOperation {
	...
	public ModifierHelper createHelpers() {
		...
		if (dataModel != null) {
			...
			EObject owner = (EObject) dataModel.getProperty(ServiceReferenceDataModel.OWNER);
			switch (dataModel.getDeploymentDescriptorType()) {
				case XMLResource.APP_CLIENT_TYPE :
					if (((ApplicationClient) owner).getVersionID() == J2EEVersionConstants.J2EE_1_4_ID) {
						...
						descriptionObj.setValue(dataModel.getStringProperty(ServiceReferenceDataModel.DESCRIPTION).trim());
						ref.getDescriptions().add(descriptionObj);
						ref.setDescription(dataModel.getStringProperty(ServiceReferenceDataModel.DESCRIPTION).trim());
					} else {
		...
		ref.setServiceRefName(dataModel.getStringProperty(ServiceReferenceDataModel.REF_NAME).trim());
		String serviceInterfaceName = dataModel.getServiceInterfaceName();
		ref.setServiceInterface((JavaClass) JavaRefFactory.eINSTANCE.reflectType(serviceInterfaceName, (EObject) dataModel.getProperty(ServiceReferenceDataModel.OWNER)));
	...
	public void createPortCompRefHelpers() {
		...
		if (dataModel != null) {
			...
			for (int i = 0; i < serEndpoints.length; i++) {
				...
				portCompRef.setServiceEndpointInterface((JavaClass) JavaRefFactory.eINSTANCE.reflectType(serviceEndpointInterface, (EObject) dataModel.getProperty(ServiceReferenceDataModel.OWNER)));
