...
public class ServerTargetHelper {
            ...
            try {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (entryKind != IClasspathEntry.CPE_SOURCE && entryKind != IClasspathEntry.CPE_LIBRARY && entryKind != IClasspathEntry.CPE_PROJECT
                            ...
                            && (entryKind != IClasspathEntry.CPE_CONTAINER || !cp[i].getPath().segment(0).equals(SERVER_CONTAINER))) {
                        list.add(cp[i]);
                    }
                }
            } catch (Exception e) {
    ...
    /*
     * public static ITargetType getV50TargetType(IProject project,
     * IServerTarget v50ServerTarget) { if (J2EENature.hasRuntime(project,
     * IEARNatureConstants.NATURE_ID)) return
     * ServerTargetManager.getTargetType(v50ServerTarget,
     * IServerTargetConstants.EAR_TYPE); if (J2EENature.hasRuntime(project,
     * IApplicationClientNatureConstants.NATURE_ID)) return
     * ServerTargetManager.getTargetType(v50ServerTarget,
     * IServerTargetConstants.EAR_TYPE); else if (J2EENature.hasRuntime(project,
     * IEJBNatureConstants.NATURE_ID)) return
     * ServerTargetManager.getTargetType(v50ServerTarget,
     * IServerTargetConstants.EJB_TYPE); else if (J2EENature.hasRuntime(project,
     * IWebNatureConstants.J2EE_NATURE_ID)) return
     * ServerTargetManager.getTargetType(v50ServerTarget,
     * IServerTargetConstants.WEB_TYPE); return null; }
     */
}