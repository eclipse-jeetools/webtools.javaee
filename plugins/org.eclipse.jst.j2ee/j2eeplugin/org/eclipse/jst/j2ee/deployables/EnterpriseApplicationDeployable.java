...
public class EnterpriseApplicationDeployable extends J2EEDeployable implements IEnterpriseApplication, ILooseArchiveSupport {
	...
	protected static IModule getModule(J2EENature nature) {
		...
		if (dep == null) {
			IModuleFactoryDelegate fac = getModuleFactory(nature);
			if (fac != null) {
	...
	protected static IModuleFactoryDelegate getModuleFactory(J2EENature nature) {
		...
		while (factories.hasNext()) {
			...
			if (!deployableFactory.getId().equals(EnterpriseApplicationDeployableFactory.ID)) {
				IModuleFactoryDelegate deployableFactoryDelegate = deployableFactory.getDelegate();
				if (deployableFactoryDelegate instanceof J2EEDeployableFactory) {
					J2EEDeployableFactory fac = (J2EEDeployableFactory) deployableFactory.getDelegate();
					if (fac.getNatureID().equals(nature.getNatureID())) {
						return fac;
					}
				}
			}
	 ...
	 * @see IEnterpriseApplication#getURI(IJ2EEModule)
	 */
	protected String getContainedURI(IModule deployable) {
		...
		if (deployable instanceof J2EEDeployable) {
			IProject aProject = ((IProjectModule) deployable).getProject();
