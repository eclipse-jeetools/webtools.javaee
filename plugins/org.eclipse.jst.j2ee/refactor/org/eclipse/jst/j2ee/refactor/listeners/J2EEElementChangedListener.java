 ...
 * ccc@us.ibm.com - classpath update if parent directory is renamed
 *******************************************************************************/
	...
	private void processJavaProject(final IJavaProject jproject, final int flags, final int kind, final IJavaElementDelta delta) {
		...
		if (ModuleCoreNature.getModuleCoreNature(project) != null) {
			...
			try {
				...
				IPath currentPathLocation = null;
				
				Object[] currentPath = null;
				for (int cnt =0; cnt < pathsToRemove.size(); cnt++)
				{
					currentPath = (Object[])pathsToRemove.get(cnt);
					currentPathLocation = ((IVirtualFolder)currentPath[1]).getWorkspaceRelativePath();
					if (currentPathLocation.isPrefixOf(outputLocation))
					{
						// the folder that the output location is in is being renamed.
						IPath suffix = outputLocation.removeFirstSegments(currentPathLocation.segmentCount());
						IPath prefix = currentPathLocation.uptoSegment(currentPathLocation.segmentCount()-1);
						currentPath=(Object[])pathsToAdd.get(cnt);
						//IPath newOutputLocation = ((IVirtualFolder)currentPath[1]).getWorkspaceRelativePath().append(suffix);
						final IPath newOutputLocation = prefix.append((IPath)currentPath[0]).append(suffix);
						WorkspaceJob job = new WorkspaceJob("J2EEComponentMappingUpdateJob") {							
							public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
								jproject.setOutputLocation(newOutputLocation, monitor);
