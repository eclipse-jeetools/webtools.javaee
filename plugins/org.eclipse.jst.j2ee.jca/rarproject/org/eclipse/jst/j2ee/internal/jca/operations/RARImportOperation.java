...
public class RARImportOperation extends J2EEImportOperation {
	 ...
	 * @param model
	 */
	public RARImportOperation(RARImportDataModel model) {
		super(model);
	}
	...
	protected void addAssociateArchivesToClassPath() {
		...
		List rarArchive = ((RARImportDataModel) getOperationDataModel()).getModuleFile().getArchiveFiles();
		...
		while (archiveList.hasNext()) {
			...
			if (anArchive != null) {
				if (anArchive.getName().endsWith(JAR_EXTENSION)) {
					ConnectorNatureRuntime connectorNature = ConnectorNatureRuntime.getRuntime(((RARImportDataModel) getOperationDataModel()).getProject());
		...
		if (!classPathEntries.isEmpty()) {
			try {
				ProjectUtilities.appendJavaClassPath(((RARImportDataModel) getOperationDataModel()).getProject(), classPathEntries);
