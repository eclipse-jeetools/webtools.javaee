...
import org.eclipse.jst.jee.internal.earcreation.DefaultJEEComponentCreationDataModelProvider;
import org.eclipse.jst.jee.internal.earcreation.IDefaultJEEComponentCreationDataModelProperties;
import org.eclipse.jst.jee.internal.earcreation.IEarFacetInstallDataModelProperties;
...
public final class EarFacetInstallPage extends DataModelFacetInstallPage implements IEarFacetInstallDataModelProperties {
	...
	public EarFacetInstallPage() {
		super("jst.ear#3.0#install.page"); //$NON-NLS-1$
	...
	protected int getJEEVersion() {
		IProjectFacetVersion version = (IProjectFacetVersion)getDataModel().getProperty(FACET_VERSION);
		return JEEVersionUtil.convertVersionStringToInt(version.getVersionString());
	}
    ...
    private void collectNewComponents(IDataModel defaultModel, List newProjects) {
        collectComponents(defaultModel.getNestedModel(IDefaultJEEComponentCreationDataModelProperties.NESTED_MODEL_EJB), newProjects);
        collectComponents(defaultModel.getNestedModel(IDefaultJEEComponentCreationDataModelProperties.NESTED_MODEL_WEB), newProjects);
        collectComponents(defaultModel.getNestedModel(IDefaultJEEComponentCreationDataModelProperties.NESTED_MODEL_CLIENT), newProjects);
        collectComponents(defaultModel.getNestedModel(IDefaultJEEComponentCreationDataModelProperties.NESTED_MODEL_JCA), newProjects);
    }
	...
	private IDataModel createNewModuleModel() {
		IDataModel defaultModel = DataModelFactory.createDataModel(new DefaultJEEComponentCreationDataModelProvider());
		// transfer properties, project name
		String projectName = model.getStringProperty(FACET_PROJECT_NAME);
		defaultModel.setProperty(IDefaultJEEComponentCreationDataModelProperties.PROJECT_NAME, projectName);
		// ear component name
		String earName = model.getStringProperty(FACET_PROJECT_NAME);
		defaultModel.setProperty(IDefaultJEEComponentCreationDataModelProperties.EAR_COMPONENT_NAME, earName);
		// ear j2ee version
		int j2eeVersion = getJEEVersion();
		defaultModel.setProperty(IDefaultJEEComponentCreationDataModelProperties.J2EE_VERSION, new Integer(j2eeVersion));
		
		IRuntime rt = (IRuntime) model.getProperty(FACET_RUNTIME);
		defaultModel.setProperty(IDefaultJEEComponentCreationDataModelProperties.FACET_RUNTIME, rt);
