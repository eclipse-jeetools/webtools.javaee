...
public class XDocletBuilder extends IncrementalProjectBuilder implements
	...
	void build(int kind, Map args, IResource resource, IContentType[] types,
		...
		if (!monitor.isCanceled() && resource.getType() == IResource.FILE) {
			buildUsingAnt(resource, monitor);
	 ...
	 * @param monitor
	 */
	private void buildUsingAnt(IResource resource, IProgressMonitor monitor) {
		URL url = Platform.getBundle(XDocletAnnotationPlugin.PLUGINID).getEntry(
		...
		try {
			...
			String beanPath = constructBeanPath(packageFragmentRoot, resource);

			Properties properties = createAntBuildProperties(resource,
			...
			AntLauncher antLauncher = new AntLauncher(url, resource.getParent()
